...
const ChartSummary = (props: PropsWithChildren<ChartSummaryProps>) => {
  ...
  return (
    <Card>
      <div>
        <div className="mb-4" style={{display: 'flex', alignItems: 'center', gap: '2rem'}}>
          <h1 style={{margin: 0}}>
      ...
      <div className={styles.details}>
        ...
        <div className="content">
          {!!datasetSummary["significantPhenotype"]?.["id"] ? (
            <Link href={`/phenotypes/${datasetSummary["significantPhenotype"]["id"]}`}>
              <span className="link primary">{datasetSummary["significantPhenotype"]?.["name"]}</span>
            </Link>
          ) : (
        ...
        <div className="content"><span>{datasetSummary["zygosity"]}</span></div>
        ...
        <div className="content">
          <span className="primary link" onClick={() => setShowMetadataModal(true)}>
            Lab conditions and equipment
          </span>
        </div>
        <div className={styles.label}>Life stage</div>
        <div className="content"><span>{datasetSummary["lifeStageName"]}</span></div>
        <div className={styles.label}>Background Strain</div>
        <div className="content"><span>{datasetSummary["geneticBackground"]}</span></div>
        <div className={styles.label}>Phenotyping center</div>
        <div className="content"><span>{datasetSummary["phenotypingCentre"]}</span></div>
