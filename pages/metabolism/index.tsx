...
const MetabolismLandingPage = () => {
  ...
  const setSelection = (parameterIndex: number, sex: 'male' | 'female', outlier: '5' | '95') => {
    ...
    <>
      ...
      <Container className="page" style={{lineHeight: 2}}>
        ...
        <Card>
          ...
          <p>
            <a className="link primary" href="https://bit.ly/IMPCMetabolism">Nature Communications publication</a>
          </p>
          <ul>
        ...
        <Card>
          <h2>Gene table</h2>
          <p>
            Mutant/wildtype ratios below the 5th percentile and above the 95th percentile of the ratio
            distributions yielded 28 gene lists that serve as a data mining resource for further
            investigation into potential links to human metabolic disorders.
          </p>
          <p>By hovering over the table you can select cells and click to explore the underlying data.</p>
          <div style={{ minWidth: '50%', textAlign: 'center', margin: '0 auto' }}>
            <table className={`table ${styles.geneTable}`}>
              ...
              <tr>
                <td></td>
                <td><FontAwesomeIcon icon={faVenus} /></td>
                <td><FontAwesomeIcon icon={faMars} /></td>
                <td><FontAwesomeIcon icon={faVenus} /></td>
                <td><FontAwesomeIcon icon={faMars} /></td>
              </tr>
              {geneTableData.map((data, index) =>
                <tr key={data[0]}>
                  <td>{data[0]}</td>
                  <td className={styles.dataCell} onClick={() => setSelection(index, 'female', '5' )}>{data[1]}</td>
                  <td className={styles.dataCell} onClick={() => setSelection(index, 'male', '5' )}>{data[2]}</td>
                  <td className={styles.dataCell} onClick={() => setSelection(index, 'female', '95' )}>{data[3]}</td>
                  <td className={styles.dataCell} onClick={() => setSelection(index, 'male', '95' )}>{data[4]}</td>
                </tr>
              )}
              </tbody>
            </table>
          </div>
          <div style={{ minWidth: '80%', textAlign: 'center', margin: '0 auto' }}>
            {!!hasSelectedACell ? (
        ...
        <Card>
          ...
          <Container>
            <Row>
              <Col>
                <img src="https://www.mousephenotype.org/data/documentation/img/more-cassette.png" alt="Illustration of the action of MORE cassettes in regulatory networks" width="100%" />
              </Col>
              <Col>
                <ul>
        ...
        <Card>
          <h2>Methods</h2>
          <p>
            Genes with phenotypes associated to the following <b>seven metabolic parameters</b>, with
            diagnostic relevance in human clinical research, were further analysed:
          </p>
          <ul>
          ...
          <p>
            Mutant/wildtype ratios (mean value of metabolic parameters of mutants divided by the mean value
            obtained for wildtypes) were calculated:
          </p>
          <ul>
